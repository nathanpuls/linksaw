export function setupEditorSafety({ dialog, prompt, snapshot, baseline, saving, save, close, quit }) {
  ...
  prompt.querySelector('[data-keep]').addEventListener('click', () => { quitting = false; prompt.hidden = true; dialog.classList.remove('confirming-close'); dialog.querySelector('#snippet-title').focus(); });
