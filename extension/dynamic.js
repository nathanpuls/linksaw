(() => {
  ...
  function applyOffset(base, offset) {
    ...
    for (const [, sign, amountText, unit] of offset.matchAll(/([+-])(\d+)([yMdhm])/g)) {
      ...
      if (unit === 'M' || unit === 'y') {
        ...
        date.setDate(1); date.setMonth(targetMonth); date.setDate(Math.min(day, daysInMonth(date.getFullYear(), date.getMonth())));
  ...
  function formatDate(date, format) {
    ...
    const values = {
      EEEE: weekdays[date.getDay()], EEE: weekdays[date.getDay()].slice(0, 3),
      ...
      HH: pad(date.getHours()), H: String(date.getHours()), hh: pad(hour12), h: String(hour12),
      mm: pad(date.getMinutes()), m: String(date.getMinutes()), ss: pad(date.getSeconds()), s: String(date.getSeconds()),
      SSS: String(date.getMilliseconds()).padStart(3, '0'), a: date.getHours() < 12 ? 'AM' : 'PM',
    };
  ...
  function expandDynamic(value, { clipboard = '', now = new Date() } = {}) {
    const marker = '\u0000CURSOR\u0000';
  ...
  globalThis.LinksawDynamic = Object.freeze({ expandDynamic });
})();
