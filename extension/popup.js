...
function render() {
  ...
  found.forEach((snippet, index) => {
    ...
    const copyButton = document.createElement('button'); copyButton.className = 'action'; copyButton.type = 'button'; copyButton.title = 'Copy'; copyButton.ariaLabel = `Copy ${label(snippet)}`; copyButton.innerHTML = icons.copy; copyButton.addEventListener('click', () => copy(content(snippet)).catch(error => status(error.message)));
    const open = document.createElement('button'); open.className = 'action'; open.type = 'button'; open.title = 'Open in Linksaw'; open.ariaLabel = `Open ${label(snippet)} in Linksaw`; open.innerHTML = icons.open; open.addEventListener('click', () => chrome.tabs.create({ url: `https://linksaw.com/app/s/${snippet.id}` }));
