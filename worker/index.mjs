...
function publicSnippetPage(snippet) {
  ...
  const html = `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="theme-color" content="#ffffff"><meta name="robots" content="noindex,nofollow"><title>${escapeHtml(pageTitle)} · Linksaw</title><link rel="icon" href="/icon.png" type="image/png"><link rel="apple-touch-icon" sizes="180x180" href="/apple-touch-icon.png"><style nonce="${nonce}">:root{color-scheme:light;font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;color:#171717;background:#fff}*{box-sizing:border-box}body{min-height:100vh;margin:0;background:#fff}.snippet{min-height:100vh;background:#fff}.header{min-height:78px;display:flex;align-items:center;gap:12px;padding:14px 22px;border-bottom:1px solid #e5e5e7}h1{flex:1;min-width:0;margin:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;font-size:21px;font-weight:600;letter-spacing:-.025em}.content{width:min(900px,100%);margin:0;padding:30px 36px 48px;white-space:pre-wrap;overflow-wrap:anywhere;font:inherit;font-size:16px;line-height:1.65}.content a{color:inherit;text-decoration-color:#d4d4d8;text-underline-offset:2px}.content a:hover,.content a:focus-visible{text-decoration-color:currentColor}.copy{width:38px;height:38px;flex:0 0 auto;display:grid;place-items:center;border:0;border-radius:9px;background:transparent;color:#777;cursor:pointer}.copy:hover,.copy:focus-visible{color:#171717}.copy:focus-visible{outline:2px solid #171717;outline-offset:2px}.copy svg{width:20px;height:20px;fill:none;stroke:currentColor;stroke-width:1.8;stroke-linecap:round;stroke-linejoin:round}.status{position:fixed;left:50%;bottom:24px;transform:translateX(-50%);padding:9px 15px;border-radius:9px;background:#171717;color:#fff;font-size:13px}@media(max-width:700px){.header{min-height:70px;padding:12px 12px 10px 16px}h1{font-size:19px}.content{padding:24px 22px 40px}}</style></head><body><main class="snippet"><header class="header"><h1${heading ? "" : " hidden"}>${escapeHtml(heading)}</h1><button id="copy" class="copy" type="button" aria-label="Copy snippet" title="Copy"><svg viewBox="0 0 24 24" aria-hidden="true"><rect width="14" height="14" x="8" y="8" rx="2" ry="2"></rect><path d="M4 16c-1.1 0-2-.9-2-2V4c0-1.1.9-2 2-2h10c1.1 0 2 .9 2 2"></path></svg></button></header><pre id="snippet-content" class="content">${publicLinkedHtml(body)}</pre></main><div id="status" class="status" role="status" hidden>Copied</div><script nonce="${nonce}">document.getElementById("copy").addEventListener("click",async()=>{try{await navigator.clipboard.writeText(document.getElementById("snippet-content").textContent);const s=document.getElementById("status");s.hidden=false;setTimeout(()=>s.hidden=true,1400)}catch{}})</script></body></html>`;
  return { html, nonce };
}
